...
def Cadastro():
    if request.method == "GET":
        ...
        if Token != "uhdfaAADF123":
            log = Webhook(Evento=f"Sistema negou o cadastro de um usuario que utilizou um token diferente do permitido!",dados=f"Token utilizado{Token}")
            db.session.add(log)
            db.session.commit()
            ...
            db.session.add(user)
            db.session.commit()
            log = Webhook(Evento=f"Usuario cadastrado com sucesso!",dados=f"Usuario:{Usuario},Email:{Email}")
            db.session.add(log)
            db.session.commit()
...
def Login():
    if request.method == "GET":
        ...
        if str(usuarioForm).lower().replace(" ","") != str(usuarioDB).lower().replace(" ",""):
            log = Webhook(Evento=f"Tentativa de login sem suceso!",dados=f"{usuarioForm}")
            db.session.add(log)
            db.session.commit()
            ...
            log = Webhook(Evento=f"Usuario logado com suceso!",dados=f"{usuarioForm}")
            db.session.add(log)
            db.session.commit()
